...
radial_dis <- function(buffer, loop, inner, exclude){   #This function first creates matrix of buffer+1,buffer+1 size and extracts values from a particular radial distance 
  m=(buffer*2)+1
  center <- buffer+1
  M <- matrix(data=NA,nrow=m,ncol=m)
  for(j in 1:m){
